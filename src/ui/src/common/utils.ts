...
export function processJson(
  ...
  try {
    if (json == null) {
      return { error: 'JSON is null or undefined' };
    }
    ...
    return { error: `Failed to process JSON: ${errorMessage}` };
  }
}
  ...
  try {
    ...
    // The json is generated by built-in adapter script.
      ...
      try {
        ...
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        return { error: `Error converting built-in adapter format: ${errorMessage}` };
      }
    }
    // The json is a list of graphs.
      ...
      } else if (json[0]?.['id'] != null && json[0]?.['nodes'] != null) {
        return {
          graphCollections: [
            {
              label: fileName,
    ...
    const formatInfo = [
      'Expected one of:',
      '1. A GraphCollection object with "label" and "graphs" properties',
      '2. An array of graphs (each with "id" and "nodes" properties)',
      '3. An array of subgraphs (for built-in adapters)'
    ].join('\n');
    
    return { error: `Unsupported JSON format. ${formatInfo}` };
  } catch (error: unknown) {
    console.error('Error in loadGraphsJson:', error);
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    return { error: `Failed to load graphs JSON: ${errorMessage}` };
  }
}
